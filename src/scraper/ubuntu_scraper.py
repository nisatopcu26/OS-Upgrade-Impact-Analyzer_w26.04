...
def scrape_version(version: str) -> list[dict]:
    ...
    for page_idx, url in enumerate(urls):
        ...
        if page_idx > 0:
            for s in sections:
                if s.get("section_id"):
                    section_id_val = s["section_id"]; s["section_id"] = f"{section_id_val}-src{page_idx}"
