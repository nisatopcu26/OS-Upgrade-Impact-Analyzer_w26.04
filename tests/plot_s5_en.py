...
Üç figür üretir (docs/s5-matrix/figs/):
  fig_s2_kalibrasyon_en.png : eşik kalibrasyon dağılımları (3 mod strip-plot)
  fig_s4_ab_en.png          : LLM A/B küçük-katlar (qwen vurgulu)
  fig_s5_matris_en.png      : 6 senaryo × 2 konfig yan yana (yeni vurgulu)
...
def fig_s2():
    ...
    panels = [
        ("MiniLM · no prefix (old)", mini["modes"]["ÖNEKSİZ"], 0.30),
        ("bge-small · no prefix", bge["modes"]["ÖNEKSİZ"], None),
        ("bge-small · PREFIXED (selected)", bge["modes"]["ÖNEKLİ"], 0.60),
    ]
    fig, axes = plt.subplots(1, 3, figsize=(9.6, 3.4), sharey=True)
    rng = np.random.default_rng(42)
    for ax, (title, d, thr) in zip(axes, panels):
        groups = [("relevant", d["alakali"], BLUE, 0),
                  ("mixed", d["karisik"], GRAY, 1),
                  ("nonsense", d["sacma"], ORANGE, 2)]
        ...
        if thr is not None:
            ...
            ax.annotate(f"thr {thr:.2f}", xy=(-0.44, thr), fontsize=8,
                        color=INK2, va="bottom", ha="left")
        rel_min, non_max = min(d["alakali"]), max(d["sacma"])
        ok = non_max < rel_min
        ax.annotate(f"rel-min {rel_min:.3f}", xy=(0, rel_min),
                    xytext=(0.28, rel_min), va="center",
                    fontsize=7.5, color=INK2)
        ax.annotate(f"nons-max {non_max:.3f}", xy=(2, non_max),
                    xytext=(1.35, non_max + 0.045), fontsize=7.5, color=INK2)
        ax.set_title(title + ("" if ok else "  —  NO GAP"),
                     fontsize=9.5, color=(INK if ok else ORANGE), pad=8)
        ax.set_xticks([0, 1, 2])
        ax.set_xticklabels(["relevant", "mixed\n(context)", "nonsense"], fontsize=8)
        ax.set_xlim(-0.5, 2.5)
        ax.grid(axis="y", color=GRID, lw=0.6, zorder=0)
        style_ax(ax)
    axes[0].set_ylabel("claim ↔ chunk cosine similarity", fontsize=8.5)
    fig.suptitle("S2 — Threshold calibration: relevant/nonsense score gap (60 faithful + 5 nonsense claims)",
                 fontsize=11, x=0.02, y=0.985, ha="left", color=INK)
    fig.text(0.02, 0.895, "in prefixless mode bge's gap closes; the query prefix opens it → threshold 0.60 (midpoint of the gap)",
             fontsize=8.5, color=INK2)
    fig.tight_layout(rect=(0, 0, 1, 0.84))
    fig.savefig(FIGS / "fig_s2_kalibrasyon_en.png", dpi=200)
    plt.close(fig)
...
def fig_s4():
    ...
    panels = [
        ("Pass rate", q["grounding_pass_rate"], l["grounding_pass_rate"], "{:.3f}", None),
        ("Verified per minute", q["verified_per_minute"], l["verified_per_minute"], "{:.2f}", None),
        ("Average duration (s)", q["avg_seconds_per_run"], l["avg_seconds_per_run"], "{:.0f} s", "lower is better"),
        ("Valid JSON runs", valid_json_runs(q), valid_json_runs(l), "{:.0f}/9", None),
    ]
    ...
    fig.suptitle("S4 — LLM A/B (new config, 3 scenarios × 3 repeats): the gap closed, qwen 2.3× faster",
                 fontsize=11, x=0.02, ha="left", color=INK)
    fig.tight_layout(rect=(0, 0, 1, 0.86))
    fig.savefig(FIGS / "fig_s4_ab_en.png", dpi=200)
    plt.close(fig)
...
def fig_s5():
    ...
    labels = {"m8_2204_2404_real": "M8 22.04→24.04 (real)",
              "m8_2004_2204_server": "M8 20.04→22.04 (server)",
              "m8_1804_2004_legacy": "M8 18.04→20.04 (legacy)",
              "ssh_2204_2404": "SSH 22.04→24.04",
              "ssh_2004_2204": "SSH 20.04→22.04",
              "ssh_1804_2004": "SSH 18.04→20.04 (469 pkgs)"}
    by = {}
    for r in results:
        if r.get("ok"):
    ...
    ax1.set_title("Verified / draft ratio", fontsize=9.5, color=INK)
    ax2.set_title("Duration (lower is better)", fontsize=9.5, color=INK)
    ax1.set_xlim(0, 1.28)
    ax2.set_xlim(0, max(by[(s, c)]["seconds"] for s in scen for c in ("eski", "yeni")) * 1.30)
    handles = [plt.Rectangle((0, 0), 1, 1, color=BLUE),
               plt.Rectangle((0, 0), 1, 1, color=GRAY)]
    fig.legend(handles, ["new (bge-small + 0.60)", "old (MiniLM + 0.30)"],
               loc="upper right", frameon=False, fontsize=8.5,
               bbox_to_anchor=(0.99, 0.97))
    fig.suptitle("S5 — Acceptance matrix: 6 scenarios × 2 configurations (LLM fixed: qwen2.5:7b)",
                 fontsize=11, x=0.02, ha="left", color=INK)
    fig.tight_layout(rect=(0, 0, 1, 0.88))
    fig.savefig(FIGS / "fig_s5_matris_en.png", dpi=200)
    plt.close(fig)
